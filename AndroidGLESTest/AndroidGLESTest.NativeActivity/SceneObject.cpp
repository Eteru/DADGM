...
SceneObject::SceneObject(Vector3 pos, Vector3 rot, Vector3 scale, bool depth_test, std::string id)
	...
	for (auto & ls : lights) {
		if (nullptr != ls.second) {
			if (objShader.lightTypeUniform[count] != -1) {
				int light_type = static_cast<int>(ls.second->GetType());
