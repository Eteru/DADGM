...
char * LoadTGA( const char * szFileName, int * width, int * height, int * bpp, const engine *eng )
{
	AAssetManager* mgr = eng->app->activity->assetManager;
	AAsset* file = AAssetManager_open(mgr, szFileName, AASSET_MODE_BUFFER);

	if (NULL == file)
		return NULL;

	TGA_HEADER header;
	AAsset_read(file, &header, sizeof(header));
	...
	long fsize = AAsset_getLength(file) - sizeof(header) - header.identsize;
	char *pBuffer = new char[fsize];

	AAsset_read(file, pBuffer, fsize);
	AAsset_close(file);
