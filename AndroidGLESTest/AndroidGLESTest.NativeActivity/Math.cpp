...
{
	return std::sqrt((x - vector.x) * (x - vector.x) + (y - vector.y) * (y - vector.y) + (z - vector.z) * (z - vector.z));
}
...
{
	GLfloat height = 2.0f * nearPlane * std::tan(fovY * 0.5f);
...
Matrix Matrix::Inverse()
{
	...
	float det = m[0][0] * inv.m[0][0] + m[0][1] * inv.m[1][0] + m[0][2] * inv.m[2][0] + m[0][3] * inv.m[3][0];
